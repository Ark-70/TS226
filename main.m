...
encod_conv = comm.ConvolutionalEncoder(...
    'TrellisStructure', treillisStruct1, ...
...
for i_snr = 1:length(EbN0dB)
    ...
    while (err_stat(2) < nbr_erreur && err_stat(3) < nbr_bit_max)
        ...
        Lc_decoded = step(decod_viter, Lc); % Décodage par Viterbi
        rec_msg = double(Lc_decoded(1:K) < 0); % D�cision
